...
let print_lambda file lambda =
  let rec print_lambda_rec lambda =
    match lambda with
    ...
    | Abs { param; body } ->
        ...
        Format.fprintf file ".";
        print_lambda_rec body;
        Format.fprintf file ")"
    | App { term; argum } ->
        Format.fprintf file "(";
        print_lambda_rec term;
        print_lambda_rec argum;
        Format.fprintf file ")"
  in
  print_lambda_rec lambda;
  Format.fprintf file "@."

let print_expr file expr =
  let rec print_expr_rec expr =
    match expr with
